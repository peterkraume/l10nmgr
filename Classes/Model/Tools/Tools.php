...
class Tools
{
    ...
    protected function getSingleRecordToTranslate($table, $uid, $previewLanguage = 0)
    {
        if (!$this->getBackendUser()->isAdmin()) {
            ...
            $fields = implode(',', array_filter(array_merge(
                $fields,
                [
                    'uid',
                    'pid',
        ...
        if ($this->t8Tools->isTranslationInOwnTable($table)) {
            ...
            $allRows = $this->getDatabaseConnection()->exec_SELECTgetRows($fields, $table,
                'uid=' . intval($uid) .
                ' AND (' .
    ...
    protected function _getFlexFormMetaDataForContentElement($table, $field, $row)
    {
        ...
        $dataStructArray = [];
        $dataStructIdentifier = GeneralUtility::makeInstance(FlexFormTools::class)->getDataStructureIdentifier($conf,
            $table,
            $field, $row);
    ...
    public function canUserEditRecord($tableName, $record)
    {
        ...
        if ($cache->has($key)) {
            ...
            if ($tableName === 'pages') {
                ...
                $permissions = $this->getBackendUser()->calcPerms($record);
                $result = ($permissions & Permission::PAGE_EDIT) && ($this->getBackendUser()->isAdmin() || (int)$record['editlock'] === 0);
            } else {
                $result = true;
                if ($record['pid'] > 0) {
                ...
                if ($result) {
                    ...
                    $dataHandler = GeneralUtility::makeInstance(DataHandler::class);
                    $dataHandler->start([], []);
                    $result = $dataHandler->checkRecordUpdateAccess($tableName, $record['uid']) && $this->getBackendUser()->recordEditAccessInternals($tableName, $record['uid']);
                }
            }
